...
TSharedPtr<FRTMPixelStreamingVideoSourceBackBufferComposited> FRTMPixelStreamingVideoSourceBackBufferComposited::Create()
{
	TSharedPtr<FRTMPixelStreamingVideoSourceBackBufferComposited> NewSource = TSharedPtr<FRTMPixelStreamingVideoSourceBackBufferComposited>(new FRTMPixelStreamingVideoSourceBackBufferComposited());
	TWeakPtr<FRTMPixelStreamingVideoSourceBackBufferComposited>	  WeakSource = NewSource;
	// Set up the callback on the game thread since FSlateApplication::Get() can only be used there
	AsyncTask(ENamedThreads::GameThread, [WeakSource]() {
		if (TSharedPtr<FRTMPixelStreamingVideoSourceBackBufferComposited> Source = WeakSource.Pin())
		{
			FSlateRenderer* Renderer = FSlateApplication::Get().GetRenderer();
			Source->DelegateHandle = Renderer->OnBackBufferReadyToPresent().AddSP(Source.ToSharedRef(), &FRTMPixelStreamingVideoSourceBackBufferComposited::OnBackBufferReady);
		} });

	return NewSource;
}

FRTMPixelStreamingVideoSourceBackBufferComposited::FRTMPixelStreamingVideoSourceBackBufferComposited()
{
	AsyncTask(ENamedThreads::GameThread, [this]() { FSlateApplication::Get().OnPreTick().AddLambda([this](float DeltaTime) {
			FScopeLock Lock(&TopLevelWindowsCS);
...
void FRTMPixelStreamingVideoSourceBackBufferComposited::OnBackBufferReady(SWindow& SlateWindow, const FTexture2DRHIRef& FrameBuffer)
{
	/**
	 ...
	 */
	{
...

		{
			...
			FRDGTextureRef OutputTexture = GraphBuilder.CreateTexture(FRDGTextureDesc::Create2D(FrameBuffer->GetDesc().Extent, FrameBuffer->GetDesc().Format, FClearValueBinding::None, ETextureCreateFlags::Shared | ETextureCreateFlags::RenderTargetable), TEXT("VideoSourceBackBufferCompositedStaging"));
			...
			GraphBuilder.AddPass(
				RDG_EVENT_NAME("VideoSourceBackBufferCopyToStaging (%s)", *SlateWindow.GetTitle().ToString()),
				PassParameters,
...
void FRTMPixelStreamingVideoSourceBackBufferComposited::CompositeWindows()
{
	FRHICommandListImmediate& RHICmdList = FRHICommandListExecutor::GetImmediateCommandList();
	...
	{ // FRDGBuilder uses a global allocator which can cause race conditions
		...
		for (FTexturedWindow& CurrentWindow : TopLevelWindows)
		{
			...
			FRDGTextureRef InputTexture = GraphBuilder.RegisterExternalTexture(CreateRenderTarget(CurrentTexture, TEXT("VideoSourceBackBufferCompositedStaging")));
			...
			AddDrawScreenPass(GraphBuilder, RDG_EVENT_NAME("VideoSourceBackBufferCompositedSwizzle"), View, OutputViewport, InputViewport, VertexShader, PixelShader, PixelShaderParameters);
		}
